...
- We propose two models: a win/loss classifier (**AUROC 91.2%**) for opportunity prioritization, and a regression model for deal sizing and aggregate forecasting (reliable at portfolio level, not per deal) with an **61.7% MdAPE**.
- We show that focusing on the **top 20% of most likely opportunities lifts win rates from the 22.7% baseline to ~74%**; while focusing on the **top 25% expected-value ranked opportunities captures ~76% of total opportunity revenue**.
...
::: aside
Selected balanced random forest for top ranking + best threshold performance, trading off interpretability.
:::
...
::: aside
Annex details model selection, calibration, thresholding (58.1% OOF); baselines use observed win rate.
:::
...
- Prioritization is the highest-impact, low-effort lever
  - Win/loss model (strong scoring performance, AUROC 91.2%) improves focus
...
- Expected value is the strongest prioritization approach
  ...
  - Revenues are concentrated **(~46% top 10% opp., ~76% top 25% opp.)** and should be prioritized accordingly
...
:::
::: {.column width="42%"}
- This chart now shows the **selected classifier itself** on the held-out set: `random_forest_classifier_balanced`.
